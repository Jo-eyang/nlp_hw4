...
def get_args():
    ...
    parser.add_argument('--augment_train', action='store_true',
                        help='Enable in-memory NL prompt augmentation for train split only')
    parser.add_argument('--augment_ratio', type=float, default=0.0,
                        help='Probability of creating one augmented NL variant per train example')
    parser.add_argument('--augment_seed', type=int, default=42,
                        help='Random seed used by train-time augmentation')
    parser.add_argument('--augment_num_augments', type=int, default=1,
                        help='How many entity-swap variants to generate per selected train example')
...
def eval_epoch(args, model, dev_loader, gt_sql_pth, model_sql_path, gt_record_path, model_record_path):
    ...
    with torch.no_grad():
        for encoder_input, encoder_mask, decoder_input, decoder_targets, _ in tqdm(dev_loader):
            ...
            # Decode generated IDs to SQL queries
    ...
    # Compute metrics (pass None for gt_query_records so it computes from SQL)
...
def main():
    ...
    train_loader, dev_loader, test_loader = load_t5_data(
        ...
        augment_num_augments=args.augment_num_augments,
    )
