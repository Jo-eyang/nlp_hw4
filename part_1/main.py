...
def do_train(args, model, train_dataloader, save_dir="./out"):
    ...
    for epoch in range(num_epochs):
        for batch in train_dataloader:
            ...
            # Optimizer step and learning rate update
            optimizer.step()
            lr_scheduler.step()
...
def create_augmented_dataloader(args, dataset):
    ...
    # Get 5000 random examples from the training set
    augment_dataset = dataset["train"].shuffle(seed=42).select(range(5000))
    
    # Apply custom transform to the selected examples
    augmented_examples = augment_dataset.map(custom_transform, load_from_cache_file=False)
    
    # Tokenize the augmented examples
    augmented_tokenized = augmented_examples.map(tokenize_function, batched=True, load_from_cache_file=False)
    ...
    tokenized_train = dataset["train"].map(tokenize_function, batched=True, load_from_cache_file=False)
    tokenized_train = tokenized_train.remove_columns(["text"])
    tokenized_train = tokenized_train.rename_column("label", "labels")
    tokenized_train.set_format("torch")
    
    # Concatenate original training set with augmented examples
    combined_dataset = datasets.concatenate_datasets([tokenized_train, augmented_tokenized])
    
    # Create dataloader from combined dataset
